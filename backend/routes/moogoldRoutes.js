...
router.post("/create-order", generalRateLimiter, authMiddleware, async (req, res) => {
  try {
    ...
    const redirectUrl = `https://zelanstore.com/api/moogold/check-status?order_id=${encodeURIComponent(order_id)}&client_txn_id=${encodeURIComponent(order_id)}&orderId=${encodeURIComponent(order_id)}`;
